...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::prefix('support/tickets')->group(function () {
        Route::get('/', [TicketController::class, 'index']);           // GET /api/support/tickets - user's tickets (x)
        Route::post('/', [TicketController::class, 'store']);          // POST /api/support/tickets - create ticke  (x)
        Route::get('/{ticket}', [TicketController::class, 'show']);    // GET /api/support/tickets/{id} (x)
        Route::put('/{ticket}', [TicketController::class, 'update']);  // PUT /api/support/tickets/{id} (x)
        Route::post('/{ticket}/reply', [TicketController::class, 'reply']); // POST /api/support/tickets/{id}/reply (x)
        Route::post('/{ticket}/close', [TicketController::class, 'close']); // POST /api/support/tickets/{id}/close (x)
    });
    ...
    Route::prefix('support/chat')->group(function () {
        ...
        // Terminate conversation (x)
        Route::post('/{conversationId}/terminate', [ChatController::class, 'terminateConversation']);
        
        // Route::get('/conversations', [ChatController::class, 'userConversations']); // GET /api/support/chat/conversations  (Wrong)
        // Route::post('/conversations', [ChatController::class, 'createConversation']); // POST /api/support/chat/conversations
        Route::get('/conversations/{conversation}', [ChatController::class, 'show']); // GET /api/support/chat/conversations/{id} (x)
        Route::get('/conversations/{conversation}/messages', [ChatController::class, 'messages']); // GET /api/support/chat/conversations/{id}/messages (x)
        ...
        Route::post('/messages', [ChatController::class, 'sendMessage']) // POST /api/support/chat/messages (x)
            ->middleware('throttle:chat-send');
    });

            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                <a href="{{ route('products.show', $product) }}">
                    @if($product->customer_images && count($product->customer_images) > 0)
                        <img src="{{ asset('storage/' . $product->customer_images[0]) }}" alt="{{ $product->name }}" class="w-full h-56 object-cover">
                    @else
                        <img src="https://placehold.co/600x400?text=Jewelry" alt="{{ $product->name }}" class="w-full h-56 object-cover">
                    @endif
                </a>
                <div class="p-4">

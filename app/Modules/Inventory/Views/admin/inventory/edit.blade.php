...
@section('content')
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" class="w-full border rounded p-2"
                            onchange="showTypeSpecificFields()">
                            <option value="RingItem" {{ $inventory->type == 'RingItem' ? 'selected' : '' }}>Ring</option>
                            <option value="NecklaceItem" {{ $inventory->type == 'NecklaceItem' ? 'selected' : '' }}>Necklace
                            </option>
                            <option value="EarringsItem" {{ $inventory->type == 'EarringsItem' ? 'selected' : '' }}>
                                Earrings</option>
                            <option value="BraceletItem" {{ $inventory->type == 'BraceletItem' ? 'selected' : '' }}>
                                Bracelet</option>
                        </select>
                    </div>
                </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="necklace_length" class="w-full border rounded p-2">
                                    ...
                                    @foreach ([30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80] as $length)
                                        <option value="{{ $length }}"
                                            {{ $inventory->necklace_length == $length ? 'selected' : '' }}>
                                            {{ $length }}cm</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="has_pendant" class="w-full border rounded p-2">
                                    ...
                                    <option value="1" {{ $inventory->has_pendant == 1 ? 'selected' : '' }}>Yes
                                    </option>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="earring_style" class="w-full border rounded p-2">
                                    ...
                                    <option value="Stud" {{ $inventory->earring_style == 'Stud' ? 'selected' : '' }}>Stud
                                    </option>
                                    <option value="Hoop" {{ $inventory->earring_style == 'Hoop' ? 'selected' : '' }}>Hoop
                                    </option>
                                    <option value="Drop" {{ $inventory->earring_style == 'Drop' ? 'selected' : '' }}>Drop
                                    </option>
                                    <option value="Chandelier"
                                        {{ $inventory->earring_style == 'Chandelier' ? 'selected' : '' }}>Chandelier
                                    </option>
                                    <option value="Cluster" {{ $inventory->earring_style == 'Cluster' ? 'selected' : '' }}>
                                        Cluster</option>
                                    <option value="Dangle" {{ $inventory->earring_style == 'Dangle' ? 'selected' : '' }}>
                                        Dangle</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="is_pair" class="w-full border rounded p-2">
                                    <option value="">Select Option</option>
                                    <option value="1" {{ $inventory->is_pair == 1 ? 'selected' : '' }}>Yes (Pair)
                                    </option>
                                    <option value="0" {{ $inventory->is_pair == 0 ? 'selected' : '' }}>No (Single)
                                    </option>
                                </select>
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                <select name="bracelet_clasp" class="w-full border rounded p-2">
                                    ...
                                    <option value="Standard"
                                        {{ $inventory->bracelet_clasp == 'Standard' ? 'selected' : '' }}>Standard</option>
                                    <option value="Magnetic"
                                        {{ $inventory->bracelet_clasp == 'Magnetic' ? 'selected' : '' }}>Magnetic</option>
                                    <option value="Toggle" {{ $inventory->bracelet_clasp == 'Toggle' ? 'selected' : '' }}>
                                        Toggle</option>
                                    <option value="Lobster"
                                        {{ $inventory->bracelet_clasp == 'Lobster' ? 'selected' : '' }}>Lobster</option>
                                    <option value="Box" {{ $inventory->bracelet_clasp == 'Box' ? 'selected' : '' }}>Box
                                    </option>
                                </select>
                            </div>
                            <div>
                        ...
                        @foreach ($inventory->variations as $index => $variation)
                            <div class="variation-item border rounded-lg p-4 bg-gray-50 relative">
                                ...
                                <div class="grid grid-cols-2 gap-4 mb-3">
                                    <div>
                                        ...
                                        <input type="text" name="variations[{{ $index }}][sku]"
                                            value="{{ $variation->sku }}" class="w-full border rounded p-2 sku-input">
                                    </div>
                                    <div>
                                        <label class="block font-medium">Color</label>
                                        <input type="text" name="variations[{{ $index }}][color]"
                                            value="{{ $variation->color }}" class="w-full border rounded p-2">
                                    </div>
                                </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4 mb-3">
                                        <div>
                                            ...
                                            <select name="variations[{{ $index }}][size]"
                                                ...
                                                @for ($i = 4; $i <= 10; $i++)
                                                    <option value="{{ $i }}"
                                                        {{ $variation->size == $i ? 'selected' : '' }}>Size
                                                        {{ $i }}</option>
                                                @endfor
                                            </select>
                                        </div>
                                        <div>
                                            ...
                                            <select name="variations[{{ $index }}][material]"
                                                class="w-full border rounded p-2">
                                                <option value="">Select Stone</option>
                                                @foreach (['Diamond', 'Ruby', 'Sapphire', 'Emerald', 'Pearl', 'Amethyst'] as $stone)
                                                    <option value="{{ $stone }}"
                                                        {{ $variation->material == $stone ? 'selected' : '' }}>
                                                        {{ $stone }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4 mb-3">
                                        <div>
                                            ...
                                            <input type="text" name="variations[{{ $index }}][size]"
                                                value="{{ $variation->size }}" class="w-full border rounded p-2">
                                        </div>
                                        <div>
                                            <label class="block font-medium">Material</label>
                                            <input type="text" name="variations[{{ $index }}][material]"
                                                value="{{ $variation->material }}" class="w-full border rounded p-2">
                                        </div>
                                    </div>
                                @endif
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="file" name="variations[{{ $index }}][image_path]"
                                        class="w-full border rounded p-2">
                                ...
                                <!-- Delete marker -->
                                <input type="hidden" name="variations[{{ $index }}][delete]" value="0"
                                    class="delete-variation-flag">
    ...
    <script>
        ...
        function updateVariationPrice(variation) {
            ...
            if (priceInput) {
                ...
                console.log('Updating variation price in edit:', { material, size, variation: variation });
            }
        }
    ...
    <div class="variation-item border rounded-lg p-4 bg-gray-50 relative">
        ...
        <div class="grid grid-cols-2 gap-4 mb-3">
            <div>
                <label class="block font-medium">Price (RM)</label>
                <input type="number" step="0.01" name="variations[${index}][price]" class="mt-1 w-full border-gray-300 rounded-lg shadow-sm bg-gray-100 cursor-not-allowed">
            </div>
            <div>
                <label class="block font-medium">Stock</label>

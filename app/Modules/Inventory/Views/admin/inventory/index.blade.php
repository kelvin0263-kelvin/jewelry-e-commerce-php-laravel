                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($inventories as $inv)
                            <tr class="hover:bg-gray-50 transition duration-150">
                                ...
                                <!-- Quantity -->
                                <td
                                    class="px-4 py-3 text-sm {{ $inv->total_quantity <= $inv->min_stock_level ? 'text-red-600 font-bold' : 'text-gray-700' }}">
                                ...
                                <!-- Status -->
                                <td class="px-4 py-3 text-sm">
                                    <form action="{{ route('admin.inventory.toggleStatus', $inv->id) }}" method="POST"
                                        class="inline-block">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit"
                                                ...
                                                <tbody class="bg-white divide-y divide-gray-200">
                                                    @foreach($inv->variations as $var)
                                                        <tr class="hover:bg-gray-50 text-xs">
                                                            ...
                                                            <td class="px-3 py-2">
                                                                @if($var->image_path)
                                                                    <img src="{{ asset($var->image_path) }}" alt="Variation Image"
                                ...
                                <td class="px-4 py-3 text-sm flex gap-2">
                                    ...
                                    <form id="edit-form-{{ $inv->id }}"
                                        action="{{ route('admin.inventory.toggleStatus', $inv->id) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('PUT')
                                    ...
                                    <form id="delete-form-{{ $inv->id }}"
                                        action="{{ route('admin.inventory.destroy', $inv->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')

...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function listenForRealTimeMessages(conversationId) {
        if (window.Echo && conversationId) {
            ...
            // Join new channel
            window.currentChannel = 'chat.' + conversationId;
    ...
    // Fallback to polling-based real-time for admin
    function usePollingFallbackAdmin() {
        if (window.PollingEcho && window.currentChannel) {
            console.log('Using polling-based real-time system for admin');
            window.PollingEcho.private(window.currentChannel)
                .listen('MessageSent', (e) => {
                    console.log('New message received in admin via polling:', e.message);
                    addMessageToContainer(e.message);

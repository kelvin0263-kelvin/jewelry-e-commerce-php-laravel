...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (storedConversationId) {
        ...
        fetch(`/chat/conversations/${storedConversationId}`)
            ...
            .catch(error => {
                ...
                localStorage.removeItem('activeConversationId');
                console.log('Cleared unvalidatable conversation from localStorage:', storedConversationId);
            });
    }
    ...
    async function startQueueChat() {
        ...
        try {
            ...
            } else if (data.existing_conversation && data.status === 'active' && !data.ended_at && !data.assigned_agent_id) {
            ...
            } else if (data.existing_conversation && (data.status === 'completed' || data.status === 'abandoned' || data.ended_at)) {
    ...
    window.fetchMessages = async function fetchMessages() {
        ...
        try {
            ...
            if (chatMessages) {
                chatMessages.innerHTML = `<p style="color: red;">Failed to load messages: ${error.message}</p>`;
            }
        }
    };
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.message && data.message.includes('terminated')) {
                        ...
                        chatInput.value = messageText; // Restore message if failed for other reasons
                    }
    ...
    window.listenForMessages = function listenForMessages() {
        ...
        channel.subscribed(() => {
                ...
                if (window.messageRefreshInterval) {
                    clearInterval(window.messageRefreshInterval);
                    window.messageRefreshInterval = null;
                    console.log('🎯 [CUSTOMER] Cleared fallback refresh on successful subscription');
                }
            })
            ...
            channel.subscription.bind('MessageSent', (data) => {
                ...
                if (data.message) {
                    ...
                    console.log('📝 Message details:', {
                        ...
                        messageUserId: data.message.user ? data.message.user.id : 'no user',
                        messageType: data.message.message_type || 'normal',
                        messageBody: data.message.body,
                        isFromCurrentUser: data.message.user ? (data.message.user.id === currentUserId) : false,
                    ...
                    if (data.message.user && data.message.user.id !== currentUserId) {
                        ...
                        console.log('🔄 [CUSTOMER] Message user ID:', data.message.user.id, 'vs Current user ID:', currentUserId);
                        ...
                        console.log('🔄 [CUSTOMER] Reason - User match:', data.message.user ? (data.message.user.id === currentUserId) : 'no user object');
                    }
                } else {
        ...
        .then(data => {
            if (data.success) {
                ...
                if (chatMessages) {
                    ...
                    terminationMessage.textContent = 'Chat has been terminated by you. Thank you for contacting us!';
...
function handleConversationTerminated(data, terminatedBy) {
    ...
    if (chatMessages) {
        ...
        terminationOverlay.innerHTML = `
            <div style="background: white; border: 2px solid #ef4444; border-radius: 12px; padding: 24px; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.1); max-width: 300px;">
